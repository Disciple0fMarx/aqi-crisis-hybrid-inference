...
def clean_air_quality_data(input_filename="city_day.csv", city_name='Delhi'):
    # Define paths using os.path for cross-platform compatibility
    raw_path = os.path.join("data", "raw", input_filename)
    processed_dir = os.path.join("data", "processed")
    output_path = os.path.join(processed_dir, f"cleaned_{'_'.join(city_name.lower().split(' '))}_data.csv")
    ...
    df_city[pollutants] = df_city[pollutants].interpolate(method='time')
    
    # 4. Final Cleanup
    # Remove rows where data is still missing (typically at the very beginning of 2015)
    df_city = df_city.dropna(subset=['PM2.5', 'AQI'])
    ...
    print(f"✓ Final Record Count: {len(df_city)}")
